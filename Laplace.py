...
def steady_state(error, tolerance, nx, ny, old_temp):
    ...
    while error > tolerance:
        ...
        for i in range(1, nx - 1):
            ...
            for j in range(1, ny - 1):
                ...
                term3 = (old_temp[i + 1, j] + T[i - 1, j]) / dx**2
                term4 = (old_temp[i, j + 1] + T[i, j - 1]) / dy**2
                T[i, j] = term3 / k + term4 / k

        # calculates convergence error
        error = np.max(np.abs(old_temp - T))
        ...
        if iteration % 10:
            ...
            # Plotting of results
            plt.figure(2)
            plt.contourf(T, levels=10)
            plt.gca().invert_yaxis()
            plt.colorbar()
            title_text = f'Laplace iterations = {iteration}'
...
def run_data():
    ...
    # function calling
    T_result = steady_state(error, tolerance, nx, ny, T)
